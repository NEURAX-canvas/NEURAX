...
**Design — RNN and diffusion**
- **A stacked, bidirectional LSTM/GRU fed layer 2+ the wrong input width.** Real `nn.LSTM`/`nn.GRU` with `num_layers>1` and `bidirectional=True` feeds every layer past the first the previous layer's *concatenated* forward+backward output (`hidden*2` wide); the stacking fix from the prior release used plain `hidden` unconditionally, correct only for the unidirectional case. Fixed in both the Rust formula (`architecture/mod.rs`) and the frontend's own verification copy (`modelCodeGen.ts`).
- **`unet_encoder`/`unet_decoder` nodes were costed with an LSTM formula.** Both names end in "encoder"/"decoder", and no earlier rule in the frontend's type-routing cascade caught them, so they fell to the generic `.endsWith('encoder'/'decoder')` fallback — an image U-Net stage priced as a recurrent cell. Fixed the routing, and added `channels`/`num_res_blocks` as accepted aliases for `block_out_channels`/`layers_per_block` (the real names these templates write), extending the `DownBlock`/`UpBlock` formulas to cost one stage transition per consecutive channel-count pair instead of a single flat in/out guess.

**Simulate — hardware, roofline, and cost, none of which had external validation before this**
- **Memory time was `compute_time_ms * 0.5`** — a flat ratio of compute time, unrelated to how many bytes an operation actually moves. Two models with identical FLOPs but very different weight footprints reported identical memory time, latency, throughput and cost. Fixed by carrying each operator's real `param_count`/`activation_memory` (already computed, previously discarded) through to a real bytes-over-bandwidth calculation.
- **The "Industrial Roofline" model's own overlap and kernel-launch-overhead fields were built and never read.** Latency took the plain max of compute and memory time (assuming 100% overlap, which no real GPU achieves) and added zero cost for kernel launches regardless of count. Now applied for real; `kernel_launch_count` itself switched from a flat `layers.len()*2` guess to the real number of atomic operations the model decomposes into. Removed `hardware/roofline.rs`'s `RooflineAnalysis` — an entirely separate, unused duplicate of logic that already lives, tested, in the hardware pass.
- **Convolution-family layers (Conv, GAN's Generator/DiscriminatorBlock, diffusion's UnetBlock/DownBlock/UpBlock/MidBlock/VaeEncoder/VaeDecoder, MoE's expert/router) were priced at 100% GPU efficiency** — they fell outside the attention/mlp efficiency weighting entirely and used the "nothing recognized, no penalty" neutral fallback. Verified against NVIDIA's own published ResNet-50 v1.5 inference throughput (single A100, mixed precision): NEURAX predicted ~7.2x too fast. Added a real, calibrated `conv_efficiency` term; GAN and diffusion blocks reuse it since they already call the identical Conv formula under the hood.
- **`gpu_utilization` could exceed 100%** on any multi-GPU configuration — the numerator used pre-division compute time while the denominator (`latency_ms`) used the per-GPU figure. An 8-GPU stable-diffusion config reported 486% utilization.
- **Zero external validation existed for latency, throughput, or energy** — every prior hardware/cost test checked only that a number was positive and finite. Added three real published references: Narayanan et al. 2021 (Megatron-LM) for multi-GPU training-scaling degradation and small-scale transformer throughput, NVIDIA's NGC ResNet-50 numbers for single-GPU CNN inference, and Luccioni et al. 2023's BLOOM carbon-footprint report for the cost pass's energy formula (0.62% off the real 433,196 kWh, given the real measured training duration as input).

**Optimize — every recommendation's stated impact was a hardcoded string**
- **Gradient-checkpointing savings were a flat 70% of activation memory** regardless of model depth, contradicting the diagnostic right next to it, which already correctly describes the technique as "roughly a square-root reduction." Replaced with the real depth-dependent fraction (Chen et al. 2016, *Training Deep Nets with Sublinear Memory Cost*): `1 - 1/sqrt(num_layers)`.
