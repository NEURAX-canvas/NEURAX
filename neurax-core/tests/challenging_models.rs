...
fn test_gpt4_analysis() {
    ...
    assert!(
        report.arch.metrics.total_parameters > 1_000_000_000u64,
        "GPT-4 should have >1B params"
    );
    assert!(
        report.arch.metrics.num_layers >= 10,
        "GPT-4 should have at least 10 layers"
    );

    // Verify FLOPs estimation
    assert!(
        report.compute.metrics.forward_flops > 1e12,
        "Forward FLOPs should be >1e12"
    );
    assert!(
        report.compute.metrics.backward_flops > report.compute.metrics.forward_flops,
        "Backward FLOPs should exceed forward"
    );

    // Verify memory estimation
    assert!(
        report.memory.metrics.peak_vram_bytes > 0,
        "Peak memory should be calculated"
    );
    ...
    println!(
        "  - Forward FLOPs: {:.2e}",
        report.compute.metrics.forward_flops
    );
    println!(
        "  - Backward FLOPs: {:.2e}",
        report.compute.metrics.backward_flops
    );
    println!(
        "  - Peak Memory: {:.2} GB",
        report.memory.metrics.peak_vram_gb()
    );
    println!(
        "  - Training Cost: ${:.2}M",
        report.cost.metrics.training_cost_usd / 1e6
    );
}
...
fn test_mixtral_moe_analysis() {
    ...
    assert!(
        report.compute.metrics.forward_flops > 0.0,
        "Forward FLOPs should be calculated"
    );

    // Verify memory includes expert overhead
    assert!(
        report.memory.metrics.peak_vram_bytes > 0,
        "Peak memory should include expert overhead"
    );
...
fn test_deepseek_extreme_moe() {
    ...
    assert!(
        report.memory.metrics.peak_vram_bytes > 0,
        "Peak memory should handle large model"
    );

    // Verify FLOPs estimation
    assert!(
        report.compute.metrics.forward_flops > 0.0,
        "FLOPs should be calculated"
    );
...
fn test_mamba_ssm_analysis() {
    ...
    assert!(
        report.arch.metrics.total_parameters > 0,
        "Should calculate SSM parameters"
    );
    ...
    assert!(
        report.memory.metrics.peak_vram_bytes > 0,
        "Peak memory should include SSM state"
    );

    // Verify layer count
    assert_eq!(
        report.arch.metrics.num_layers, 64,
        "Mamba should have 64 layers"
    );
...
fn test_precision_levels() {
    ...
    println!("\nPrecision Analysis:");
    println!(
        "  GPT-4:      Params={:.2e}, FLOPs={:.2e}",
        gpt4_result.arch.metrics.total_parameters, gpt4_result.compute.metrics.forward_flops
    );
    println!(
        "  Mixtral:    Params={:.2e}, Layers={}",
        mixtral_result.arch.metrics.total_parameters, mixtral_result.arch.metrics.num_layers
    );
    println!(
        "  DeepSeek:   Params={:.2e}, Layers={}",
        deepseek_result.arch.metrics.total_parameters, deepseek_result.arch.metrics.num_layers
    );
    println!(
        "  Mamba:      Params={:.2e}, Custom=Yes",
