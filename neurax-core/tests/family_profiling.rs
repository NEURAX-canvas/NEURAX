...
fn test_f02_batchnorm_params() {
    ...
    println!(
        "✓ F02 BatchNorm: params = 2×C = {} (correct)",
        expected_params
    );
}
...
fn test_f02_conv2d_flops_formula() {
    ...
    assert_eq!(
        expected_flops, 59_006_976,
        "F02 Conv2D FLOPs formula incorrect"
    );
    println!(
        "✓ F02 Conv2D: FLOPs = 2 × Cout × H × W × (Cin × Kh × Kw) = {}",
        expected_flops
    );
}
...
fn test_f02_gcn_flops() {
    ...
    assert_eq!(
        expected_flops, 16_384_000,
        "F02 GCN FLOPs formula incorrect"
    );
    println!(
        "✓ F02 GCN: FLOPs ≈ 2 × N × D_in × D_out = {}",
        expected_flops
    );
}
...
fn test_f02_linear_flops() {
    ...
    assert_eq!(
        expected_flops, 4_718_592,
        "F02 Linear FLOPs formula incorrect"
    );
    println!(
        "✓ F02 Linear: FLOPs = 2 × batch × in × out = {}",
        expected_flops
    );
}
...
fn test_f02_attention_flops() {
    ...
    println!(
        "✓ F02 Attention: FLOPs = {:.3e}",
        total_attention_flops as f64
    );
