...
fn test_f03_effective_tflops_calculation() {
    ...
    assert!(
        (effective_tflops - 234.0f64).abs() < 1.0,
        "F03: Effective TFLOPS calculation incorrect"
    );

    println!(
        "✓ F03: Effective TFLOPS = {:.1} (peak {:.1} × eff {:.0}%)",
        effective_tflops,
        peak_tflops,
        efficiency * 100.0
    );
...
fn test_f03_roofline_model() {
    ...
    assert!(
        (ridge_point - 153.0f64).abs() < 5.0,
        "F03: Ridge point calculation incorrect"
    );

    println!(
        "✓ F03: Roofline ridge point = {:.0} FLOPs/byte",
        ridge_point
    );
}
...
fn test_f04_tied_embeddings_not_doubled() {
    ...
    assert_eq!(
        savings,
        vocab_size * d_model,
        "F04: Tied embeddings savings incorrect"
    );
...
fn test_f04_gpt2_has_tied_embeddings() {
    ...
    println!(
        "✓ F04: GPT-2 params = {} (tied embeddings config may vary)",
        total_params
    );
}
...
fn test_f05_kernel_overhead_included() {
    ...
    assert!(
        (total_overhead_ms - 0.5).abs() < 0.01,
        "F05: Kernel overhead calculation incorrect"
    );
    println!(
        "✓ F05: Kernel overhead = {:.2} ms for {} kernels",
        total_overhead_ms, num_kernels
    );
}
...
fn test_f05_latency_includes_overhead() {
    ...
    println!(
        "✓ F05: Latency = {:.2} ms (includes kernel overhead)",
        latency_ms
    );
}
...
fn test_f05_overhead_significant_small_batch() {
    ...
    assert!(
        overhead_ratio > 0.1,
        "F05: Overhead should be significant for small batch"
    );

    println!(
        "✓ F05: Small batch overhead ratio = {:.1}%",
...
fn test_f03_f07_all_implemented() {
    ...
    assert!(
        result.compute.metrics.forward_flops > 0.0,
        "forward_flops missing"
    );
    assert!(
        result.compute.metrics.backward_flops > 0.0,
        "backward_flops missing"
    );
    assert!(
        result.arch.metrics.total_parameters > 0,
        "total_parameters missing"
    );
    assert!(
        result.memory.metrics.peak_vram_bytes > 0,
        "peak_vram_bytes missing"
    );
    assert!(
