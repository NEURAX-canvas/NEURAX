...
fn test_a03_layer_count_consistency() {
    ...
    assert!(
        ...
        result.arch.metrics.num_layers,
        result.arch.layers.len()
    );
    println!(
        "✓ A03: num_layers = layers.len() = {}",
        result.arch.metrics.num_layers
    );
}
...
fn test_a09_flops_per_token_coherence() {
    ...
    println!(
        "✓ A09: flops_per_token × {} = {:.3e} ≈ forward_flops = {:.3e}",
...
fn test_a10_macs_coherence() {
    ...
    assert!(
        ...
        result.compute.metrics.macs,
        expected_macs
    );
    println!(
        "✓ A10: macs = {:.3e} = forward_flops/2",
        result.compute.metrics.macs
    );
}
...
fn test_a12_vram_training_gte_inference() {
    ...
    assert!(peak_vram > 0, "A12: peak_vram should be > 0");
...
fn test_a13_vram_inference_gte_params() {
    ...
    assert!(
        ...
        result.memory.metrics.peak_vram_bytes,
...
fn test_a14_vram_optimizer_adam() {
    ...
    let relative_error =
        (result.memory.metrics.optimizer_state_bytes as f64 - expected_optimizer as f64).abs()
    ...
    println!(
        "✓ A14: optimizer_state = {} bytes ≈ 2× param_memory",
        result.memory.metrics.optimizer_state_bytes
    );
}
...
fn test_a16_vram_training_sum_components() {
    ...
    let relative_error = ((result.memory.metrics.peak_vram_bytes as i64 - expected_peak as i64)
        .abs() as f64)
...
fn test_a18_vram_inference_finite() {
    ...
    println!(
        "✓ A18: peak_vram = {} bytes (finite and positive)",
        result.memory.metrics.peak_vram_bytes
    );
}
...
fn test_a22_gpu_utilization_range() {
    ...
    assert!(
        result.hardware.metrics.gpu_utilization >= 0.0
            && result.hardware.metrics.gpu_utilization <= 1.0,
        "A22: gpu_utilization should be in [0, 1]"
    );
    println!(
        "✓ A22: gpu_utilization = {:.1}%",
...
fn test_a23_effective_tflops_nonnegative() {
    ...
    println!(
        "✓ A23: effective_tflops = {:.1}",
        result.hardware.metrics.effective_tflops
    );
}
...
fn test_a24_roofline_position_range() {
    ...
    assert!(
        result.hardware.metrics.roofline_position >= 0.0
            && result.hardware.metrics.roofline_position <= 1.0,
        "A24: roofline_position should be in [0, 1]"
    );
    println!(
        "✓ A24: roofline_position = {:.2}",
        result.hardware.metrics.roofline_position
    );
}
...
fn test_a25_bottleneck_valid() {
    ...
    assert!(
        bottleneck_str == "ComputeBound"
            || bottleneck_str == "MemoryBound"
            || bottleneck_str == "Balanced",
...
fn test_a34_flops_latency_consistency() {
    ...
    if latency_seconds > 0.0 && result.hardware.metrics.effective_tflops > 0.0 {
        ...
        println!(
            "✓ A34: flops/latency ≈ effective_tflops (ratio = {:.1})",
            ratio
        );
    } else {
        ...
        println!(
            "✓ A34: forward_flops = {:.3e} (effective_tflops = 0)",
            result.compute.metrics.forward_flops
        );
    }
}
...
fn test_a35_total_step_flops_sum() {
    ...
    let relative_error =
        (result.compute.metrics.total_step_flops - expected_total).abs() / expected_total;

    assert!(
        relative_error < 0.01,
        "A35: total_step_flops = {:.3e} ≠ sum = {:.3e}",
        result.compute.metrics.total_step_flops,
        expected_total
    );
...
fn test_a36_operator_count_positive() {
    ...
    println!(
        "✓ A36: operator_count = {}",
        result.operator.metrics.total_op_count
    );
}
...
fn test_a37_tensor_count_positive() {
    ...
    println!(
        "✓ A37: tensor_count = {}",
        result.tensor.metrics.total_tensor_count
    );
}
...
fn test_a38_graph_edge_count_positive() {
    ...
    println!(
        "✓ A38: graph_edge_count = {}",
        result.graph.metrics.edge_count
    );
}
...
fn test_a39_graph_total_operations_nonnegative() {
    ...
    println!(
        ...
        result.graph.metrics.total_operations
    );
}
