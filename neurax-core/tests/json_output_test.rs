...
fn test_json_output_gpt3() {
    ...
    assert!(
        json.contains("total_parameters"),
        "JSON should contain total_parameters"
    );
    assert!(
        json.contains("num_layers"),
        "JSON should contain num_layers"
    );
    assert!(
        json.contains("forward_flops"),
        "JSON should contain forward_flops"
    );
    assert!(
        json.contains("peak_vram_bytes"),
        "JSON should contain peak_vram_bytes"
    );
    assert!(
        json.contains("training_cost_usd"),
        "JSON should contain training_cost_usd"
    );
...
fn test_json_output_to_file() {
    ...
    assert!(
        saved_content.contains("total_parameters"),
        "Saved JSON should contain metrics"
    );
...
fn test_json_output_all_metrics_present() {
    ...
    assert!(
        ...
        "JSON should have metrics object"
    );
    ...
    // Structure metrics
    assert!(
        metrics.get("structure").is_some(),
        "Should have structure metrics"
    );

    // Compute metrics
    assert!(
        metrics.get("compute").is_some(),
        "Should have compute metrics"
    );

    // Memory metrics
    assert!(
        metrics.get("memory").is_some(),
    ...
    // Hardware metrics
    assert!(
        metrics.get("hardware").is_some(),
        "Should have hardware metrics"
    );
...
fn test_json_output_schema_version() {
    ...
    assert!(
        parsed.get("neurax_version").is_some(),
        "JSON should have neurax_version"
    );
