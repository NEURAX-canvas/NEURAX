...
fn test_vram_components_sum() {
    ...
    println!(
        "✓ VRAM components sum: peak={} bytes, components={} bytes",
        peak_vram, sum_components
    );
}
...
fn test_param_memory_consistency() {
    ...
    println!(
        "✓ Param memory consistency: {} bytes for {} params ({:.1} bytes/param)",
...
fn test_gradient_memory_equals_params() {
    ...
    println!(
        "✓ Gradient memory = param memory: {} bytes",
        gradient_memory
    );
}
...
fn test_optimizer_state_adamw() {
    ...
    // Tolérance ±10%
    let error = ((optimizer_memory as i64 - expected_optimizer as i64).abs() as f64)
    ...
    println!(
        "✓ Optimizer state (AdamW): {} bytes = 2× params",
        optimizer_memory
    );
}
...
fn test_max_batch_size_fits_vram() {
    ...
    println!(
        "✓ Max batch size fit: {} (peak_vram={}, gpu_memory={:.1}GB)",
        max_batch, peak_vram, gpu_memory_gb
    );
}
