...
fn bench_conv2d_params(c: &mut Criterion) {
    c.bench_function("conv2d_params_resnet_first", |b| {
        b.iter(|| {
            neurax_formulas::conv::conv2d_params(
                black_box(3),
                black_box(64),
                black_box(7),
                black_box(7),
...
fn bench_attention_flops(c: &mut Criterion) {
    ...
    for (name, params) in [
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(name),
            &params,
            |b, &(batch, seq, hidden, heads)| {
                b.iter(|| {
                    neurax_formulas::attention::attention_flops(
                        black_box(batch),
                        black_box(seq),
                        black_box(hidden),
                        black_box(heads),
...
fn bench_gqa_flops(c: &mut Criterion) {
    ...
    for (name, params) in [
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(name),
            &params,
            |b, &(batch, seq, hidden, heads, kv_heads)| {
                b.iter(|| {
                    neurax_formulas::attention::gqa_flops(
                        black_box(batch),
                        black_box(seq),
                        black_box(hidden),
                        black_box(heads),
...
fn bench_mlp_flops(c: &mut Criterion) {
    ...
    for (name, params) in [
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(name),
            &params,
            |b, &(batch, seq, hidden, intermediate)| {
                b.iter(|| {
                    neurax_formulas::mlp::mlp_flops(
                        black_box(batch),
                        black_box(seq),
                        black_box(hidden),
...
fn bench_gated_mlp_flops(c: &mut Criterion) {
    c.bench_function("gated_mlp_llama", |b| {
        b.iter(|| {
            neurax_formulas::mlp::gated_mlp_flops(
                black_box(1),
                black_box(2048),
                black_box(4096),
                black_box(11008),
...
fn bench_mamba_flops(c: &mut Criterion) {
    ...
    for (name, params) in [
        ...
        group.bench_with_input(
            BenchmarkId::from_parameter(name),
            &params,
            |b, &(batch, seq, hidden, state, expand)| {
                b.iter(|| {
                    neurax_formulas::ssm::mamba_flops(
                        black_box(batch),
                        black_box(seq),
                        black_box(hidden),
                        black_box(state),
...
fn bench_lstm_flops(c: &mut Criterion) {
    c.bench_function("lstm_flops", |b| {
        b.iter(|| {
            neurax_formulas::rnn::lstm_flops(
                black_box(32),
                black_box(128),
                black_box(256),
                black_box(256),
