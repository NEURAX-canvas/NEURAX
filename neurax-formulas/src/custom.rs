...
const ALLOWED_FUNCTIONS: &[&str] = &[
    "sqrt", "abs", "min", "max", "pow", "log", "exp", "sin", "cos", "tan", "floor", "ceil", "round",
];
...
impl CustomEquationEvaluator {
    ...
    pub fn validate_equation_syntax(equation: &str) -> Result<(), String> {
        ...
        let dangerous = [
            "import", "include", "exec", "eval", "system", "file", "read",
        ];
...
pub fn evaluate_custom_equation(equation: &str, variables: &[(&str, f64)]) -> Result<f64, String> {
...
mod tests {
    ...
    fn test_validate_equation() {
        ...
        assert!(
            CustomEquationEvaluator::validate_equation("").is_err()
                || CustomEquationEvaluator::validate_equation("2 * B * S").is_ok()
        );
    }
}
