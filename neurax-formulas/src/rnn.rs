...
/// h' = o * tanh(c')
#[inline(always)]
pub fn lstm_flops(batch: usize, seq_len: usize, hidden_size: usize, input_size: usize) -> f64 {
    ...
    let gate_flops =
        4.0 * 2.0 * batch as f64 * seq_len as f64 * combined_dim as f64 * hidden_size as f64;
...
#[inline(always)]
pub fn gru_flops(batch: usize, seq_len: usize, hidden_size: usize, input_size: usize) -> f64 {
    let combined_dim = hidden_size + input_size;

    // 2 gates: update (z), reset (r)
    let gate_flops =
        2.0 * 2.0 * batch as f64 * seq_len as f64 * combined_dim as f64 * hidden_size as f64;

    // Candidate hidden state: tanh(Wh * [r*h, x])
    let candidate_dim = hidden_size + input_size;
    let candidate_flops =
        2.0 * batch as f64 * seq_len as f64 * candidate_dim as f64 * hidden_size as f64;
...
#[inline(always)]
pub fn rnn_flops(batch: usize, seq_len: usize, hidden_size: usize, input_size: usize) -> f64 {
    // h' = tanh(Wh * h + Wx * x + b)
