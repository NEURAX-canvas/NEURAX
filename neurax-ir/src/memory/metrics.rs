...
pub fn memory_breakdown(metrics: &MemoryMetrics) -> MemoryBreakdown {
    ...
    MemoryBreakdown {
        params_pct: if total > 0.0 {
            metrics.parameter_memory_bytes as f64 / total * 100.0
        } else {
            0.0
        },
        activations_pct: if total > 0.0 {
            metrics.activation_memory_bytes as f64 / total * 100.0
        } else {
            0.0
        },
        gradients_pct: if total > 0.0 {
            metrics.gradient_memory_bytes as f64 / total * 100.0
        } else {
            0.0
        },
        optimizer_pct: if total > 0.0 {
            metrics.optimizer_state_bytes as f64 / total * 100.0
        } else {
            0.0
        },
    }
...
pub fn oom_risk_description(risk: OomRisk) -> &'static str {
    match risk {
        ...
        OomRisk::Warning => {
            "Memory usage is high (80-95% of GPU VRAM). Consider gradient checkpointing."
        }
