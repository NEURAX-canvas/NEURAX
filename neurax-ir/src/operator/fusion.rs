...
impl FusionDetector {
    ...
    pub fn detect(&self, layers: &[LayerInfo]) -> Vec<FusionOpportunity> {
        ...
        for i in 0..layers.len().saturating_sub(2) {
            if let Some(opp) =
                self.detect_conv_bn_activation(&layers[i], &layers[i + 1], &layers[i + 2])
            {
                if opp.confidence >= self.min_confidence {
                    opportunities.push(opp);
                }
    ...
    fn detect_linear_activation(
        &self,
        l1: &LayerInfo,
        l2: &LayerInfo,
    ) -> Option<FusionOpportunity> {
        if l1.layer_type == "Linear" && is_activation(&l2.layer_type) {
            let bytes_saved = (l1.output_bytes as f64
                * FusionPattern::LinearActivation.bytes_reduction_ratio())
                as u64;
    ...
    fn detect_conv_bn_activation(
        &self,
        l1: &LayerInfo,
        l2: &LayerInfo,
        l3: &LayerInfo,
    ) -> Option<FusionOpportunity> {
        if l1.layer_type == "Conv2d"
            && l2.layer_type == "BatchNorm"
            && is_activation(&l3.layer_type)
        {
            let total_bytes = l1.output_bytes + l2.output_bytes;
            let bytes_saved = (total_bytes as f64
                * FusionPattern::ConvBnActivation.bytes_reduction_ratio())
                as u64;
            Some(FusionOpportunity {
    ...
    fn detect_mlp_block(
        ...
        if l1.layer_type == "Linear" && is_activation(&l2.layer_type) && l3.layer_type == "Linear" {
            let total_bytes = l1.output_bytes + l2.output_bytes;
            let bytes_saved =
                (total_bytes as f64 * FusionPattern::MlpBlock.bytes_reduction_ratio()) as u64;
            Some(FusionOpportunity {
    ...
    fn detect_attention_residual(
        ...
        if l1.layer_type == "Attention" && l2.layer_type == "Residual" {
            let bytes_saved = (l1.output_bytes as f64
                * FusionPattern::AttentionResidual.bytes_reduction_ratio())
                as u64;
            Some(FusionOpportunity {
    ...
    fn detect_layernorm_linear(&self, l1: &LayerInfo, l2: &LayerInfo) -> Option<FusionOpportunity> {
        ...
        {
            let bytes_saved = (l1.output_bytes as f64
                * FusionPattern::LayerNormLinear.bytes_reduction_ratio())
                as u64;
            Some(FusionOpportunity {
...
fn is_activation(layer_type: &str) -> bool {
    matches!(
        layer_type,
        "ReLU" | "GELU" | "SiLU" | "Sigmoid" | "Tanh" | "Swish"
    )
}
...
mod tests {
    ...
    fn test_detect_mlp_block() {
        ...
        assert!(opportunities
            .iter()
            .any(|o| o.pattern == FusionPattern::MlpBlock));
    }

    #[test]
    fn test_fusion_analysis() {
        let opportunities = vec![FusionOpportunity {
            pattern: FusionPattern::LinearActivation,
