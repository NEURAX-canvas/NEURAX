...
use super::{ParallelStrategy, ParallelismMetrics};
...
pub fn format_strategy(strategy: &ParallelStrategy) -> String {
    match strategy {
        ParallelStrategy::DataParallel {
            ...
            format!(
                "Data Parallel ({} GPUs, {:.1}% efficiency)",
                num_gpus,
                efficiency * 100.0
            )
        }
        ...
        ParallelStrategy::PipelineParallel {
            ...
            format!(
                "Pipeline Parallel ({} stages, {} micro-batches, {:.1}% bubble)",
                stages,
                micro_batches,
                bubble_ratio * 100.0
            )
...
pub fn get_recommendation(metrics: &ParallelismMetrics) -> String {
    if metrics.communication_overhead > 0.3 {
        "High communication overhead. Consider tensor parallelism or faster interconnect."
            .to_string()
