    ...
    fn build(
        ...
        let hidden = ctx
            ...
            .or_else(|| {
                ctx.config
                    ...
                    .layers
                    .iter()
                    .find_map(|l| l.params.hidden_size)
            })
            .unwrap_or(512);
        ...
        for &node_idx in &input.topo_order {
            if let Some(node) = input.dag.node_weight(node_idx) {
                ...
                tensor_ir.layer_tensors.insert(
                    node.layer_id.clone(),
                    LayerTensors {
...
fn propagate_shape(
    ...
    match layer_type {
            ...
            let embed_dim = config
                ...
                .or_else(|| {
                    config
                        ...
                        .layers
                        .first()
                        .and_then(|l| l.params.embedding_dim)
                })
                .unwrap_or(512);
        ...
        | LayerType::C2f
        | LayerType::Detection
        | LayerType::Transition => Shape::known(input.to_vec()),
        ...
        | LayerType::ProgressiveBlock => Shape::known(input.to_vec()),
