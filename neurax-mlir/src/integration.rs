...
use crate::dialects::{
    ArchitectureDialect, CostDialect, HardwareDialect, MemoryDialect, OperatorDialect,
    ParallelismDialect, ReportDialect,
};
...
pub fn model_to_mlir(
    ...
    let _model_op = ArchitectureDialect::model(context, model_name, model_type, location)
        .map_err(|e| format!("Failed to create model operation: {:?}", e))?;

    // Create metrics operation
    let _metrics_op = ArchitectureDialect::metrics(context, total_parameters, num_layers, location)
...
pub fn create_transformer_layer(
    ...
    let _layer_op = ArchitectureDialect::layer(context, layer_id, "attention", location)
    ...
    let _mlp_op = OperatorDialect::matmul(context, mlp_params, mlp_flops, location)
...
pub fn create_hardware_config(
    ...
    let _par_op = ParallelismDialect::hybrid(context, dp, tp, pp, location)
...
pub fn create_cost_analysis(
    ...
    let _energy_op = CostDialect::energy(context, energy_kwh, co2_kg, location)
...
mod tests {
    ...
    fn test_transformer_layer_creation() {
        ...
        let result = create_transformer_layer(ctx.as_context(), "layer_0_attn", 8192, 64, 28672);
    ...
    fn test_full_pipeline() {
        ...
        // Hardware config
        let result =
            create_hardware_config(ctx.as_context(), "H100_SXM5", 989.0, 3352.0, 80, 8, 8, 1);
        assert!(result.is_ok());

        // Cost analysis
        let result =
            create_cost_analysis(ctx.as_context(), 720.0, 144000.0, 57600.0, 50000.0, 25.0);
        assert!(result.is_ok());

        // Final report
        let result = create_report(ctx.as_context(), "Llama-3-70B", "transformer", 150);
        assert!(result.is_ok());
    }
}
