...
use crate::{
    dialects::{
        ArchitectureDialect, ComputeDialect, CostDialect, GraphDialect, HardwareDialect,
        MemoryDialect, OperatorDialect, ParallelismDialect, ReportDialect, TensorDialect,
    },
...
mod architecture {
    ...
    fn test_arch_model_operation() {
        ...
        let op =
            ArchitectureDialect::model(ctx.as_context(), "test_model", "transformer", location)
                .unwrap();
    ...
    fn test_arch_layer_operation() {
        ...
        let op =
            ArchitectureDialect::layer(ctx.as_context(), "layer_0", "attention", location).unwrap();
...
mod integration {
    ...
    fn test_llama70b_model_representation() {
        ...
        let model_op =
            ArchitectureDialect::model(ctx.as_context(), "Llama-3-70B", "transformer", location)
                .unwrap();
        assert!(model_op.to_string().contains("arch.model"));

        // Create layer operations
        let layer_op =
            ArchitectureDialect::layer(ctx.as_context(), "layer_0_attn", "attention", location)
                .unwrap();
        ...
        // Create cost metrics
        let cost_op =
            CostDialect::training_cost(ctx.as_context(), 720.0, 144000.0, 57600.0, location)
                .unwrap();
