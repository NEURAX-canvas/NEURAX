...
use neurax_mlir::{
    dialects::{
        ...
        OperatorDialect, ParallelismDialect, ReportDialect,
    },
...
fn test_transformer_model_mlir() {
    ...
    let metrics = ArchitectureDialect::metrics_full(
        context, 8030000000, // total_parameters
        32,         // num_layers
...
fn test_compute_dialect() {
    ...
    // Test metrics
    let metrics = ComputeDialect::metrics(
        context, 3.14e23, 1.57e23, 7.5e6, 127.5, 3.14e23, 6.28e23, 0.0, 9.42e23, loc,
    );
    assert!(
        metrics.is_ok(),
        "Failed to create compute metrics operation"
    );
}
...
fn test_hardware_dialect() {
    ...
    let gpu = HardwareDialect::gpu_full(
        context, "H100-SXM", 8, 80, 989.4, 1979.0, 3352.0, true, true, loc,
    );
...
fn test_report_dialect() {
    ...
    // Test diagnostic
    let diag = ReportDialect::diagnostic(
        context,
        "memory_overflow",
        "warning",
        "Peak memory exceeds 80GB",
        loc,
    );
...
fn test_full_integration_pipeline() {
    ...
    let params = ArchitectureDialect::global_params(
        context,
    ...
    let compute_metrics = ComputeDialect::metrics(
        context, 1.0e23, 5.0e22, 5.0e6, 100.0, 1.0e23, 2.0e23, 0.0, 3.0e23, loc,
    );
    assert!(compute_metrics.is_ok());

    let mem_metrics = MemoryDialect::metrics(
        context,
        28000000000,
        8000000000,
        28000000000,
    ...
    let hw_metrics =
        HardwareDialect::metrics(context, 50.0, 20000.0, 0.85, 0.72, 450.0, 2800.0, loc);
