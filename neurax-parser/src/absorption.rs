...
use crate::model_config::{
    DataConfig, GlobalParams, HardwareConfig, ModelConfig, ModelType, TrainingConfig,
};
...
impl GlobalResolutionContext {
    ...
    pub fn build(
        ...
        let head_dim =
            if let Some(hd) = global_params.extra.get("head_dim").and_then(|v| v.as_u64()) {
                hd
        ...
        let primary_gpu_mem = primary
            .and_then(|g| g.memory_gb)
            .map(|m| m as f64)
            .unwrap_or(80.0);
        let primary_gpu_bw = primary
            .and_then(|g| g.memory_bandwidth_gbs)
            .unwrap_or(2000.0);
        ...
        let tied = matches!(
            model_type,
            ModelType::Transformer | ModelType::Moe | ModelType::Ssm
        );
        ...
        // Vocab
        let vocab: Option<u64> = global_params
            .vocab_size
            .map(|v| v as u64)
        ...
        let num_classes: Option<u64> = data.num_classes.map(|v| v as u64).or(global_params
            .extra
            .get("num_classes")
            .and_then(|v| v.as_u64()));
        ...
        let critical_fields = [
            (
                "hidden_size",
                global_params.extra.contains_key("hidden_size"),
            ),
        ...
        Self {
            ...
            bidirectional_rnn: global_params
                .extra
                .get("bidirectional_rnn")
                .and_then(|v| v.as_bool())
                .unwrap_or(false),
            cell_type: global_params
                .extra
                .get("cell_type")
            ...
            noise_schedule: global_params
                .extra
                .get("noise_schedule")
                .and_then(|v| v.as_str().map(|s| s.to_string())),
            beta_start: global_params
                .extra
                .get("beta_start")
                .and_then(|v| v.as_f64()),
    ...
    pub fn resolve_shape(&self, shape: &[Dim]) -> (Vec<u64>, f32) {
        ...
        let ratio = if shape.is_empty() {
            1.0
        } else {
            resolved_count as f32 / shape.len() as f32
        };
        (resolved, ratio)
    }
    ...
    pub fn d_inner(&self) -> Option<u64> {
        self.hidden_size
            .zip(self.ssm_expand.map(|e| e as u64))
            .map(|(h, e)| h * e)
    }
}
...
impl GlobalPropagator {
    ...
    pub fn propagate(layer: &mut crate::model_config::Layer, ctx: &GlobalResolutionContext) {
...
impl<'a> DimResolver<'a> {
    ...
    fn apply_arch_rule(&self, sym: &str, ctx: &LayerDimContext) -> Option<u64> {
        match (sym, ctx.layer_type.as_str()) {
            ...
            ("d_inner", "mamba_block") => self.grc.d_inner(),
...
impl LayerParamsMap {
    pub fn from_layer(layer_params: &crate::model_config::LayerParams) -> Self {
        ...
        if let Some(v) = layer_params.hidden_size {
            values.insert("hidden_size".into(), v as u64);
        }
        if let Some(v) = layer_params.vocab_size {
            values.insert("vocab_size".into(), v as u64);
        }
        if let Some(v) = layer_params.embedding_dim {
            values.insert("embedding_dim".into(), v as u64);
        }
        if let Some(v) = layer_params.num_heads {
            values.insert("num_heads".into(), v as u64);
        }
        if let Some(v) = layer_params.in_features {
            values.insert("in_features".into(), v as u64);
        }
        if let Some(v) = layer_params.out_features {
            values.insert("out_features".into(), v as u64);
        }
        if let Some(v) = layer_params.intermediate_size {
            values.insert("intermediate_size".into(), v as u64);
        }
