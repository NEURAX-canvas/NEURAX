...
impl CoherenceValidator {
    ...
    pub fn validate(&self, value: &Value) -> CoherenceResult {
        ...
        // Get global params
        let global_params = value.get("model").and_then(|m| m.get("global_params"));
    ...
    fn embedding_params(&self, params: Option<&Value>) -> u64 {
        let vocab = params
            .and_then(|p| p.get("vocab_size"))
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        let dim = params
            .and_then(|p| p.get("embedding_dim"))
            .and_then(|v| v.as_u64())
            .unwrap_or(0);
        vocab * dim
    }
    ...
    fn attention_params(&self, params: Option<&Value>) -> u64 {
        let hidden = params
            ...
            .unwrap_or(0);
        let _heads = params
            .and_then(|p| p.get("num_attention_heads"))
            .and_then(|v| v.as_u64())
            .unwrap_or(1);
        let layers = params
    ...
    fn moe_params(&self, params: Option<&Value>) -> u64 {
        let hidden = params
            ...
            .unwrap_or(0);
        let experts = params
            .and_then(|p| p.get("num_experts"))
            .and_then(|v| v.as_u64())
            .unwrap_or(8);
        let inter = params
            .and_then(|p| p.get("intermediate_size"))
            .and_then(|v| v.as_u64())
            .unwrap_or(hidden * 4);
        let layers = params
            .and_then(|p| p.get("num_layers"))
    ...
    fn compute_flops(
        &self,
        layers: &Value,
        global_params: Option<&Value>,
        batch_size: u64,
        seq_length: u64,
        result: &mut CoherenceResult,
    ) {
            ...
            for layer in layers_arr {
                ...
                match layer_type {
                    ...
                    "conv" | "conv2d" => {
                        ...
                        let conv_flops =
                            2.0 * batch * out_channels * in_channels * kernel_size * kernel_size;
