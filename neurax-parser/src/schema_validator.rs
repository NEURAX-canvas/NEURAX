...
pub enum ValidationError {
    ...
    #[error(
        "Parameter count mismatch: computed {computed}, expected {expected} (diff: {diff:.1}%)"
    )]
    ParameterMismatch {
        computed: u64,
        expected: u64,
    ...
    #[error(
        "Parallelism mismatch: DP({dp}) x TP({tp}) x PP({pp}) = {product} != {gpu_count} GPUs"
    )]
    ParallelismMismatch {
        dp: u32,
        tp: u32,
...
impl ModelValidator {
    ...
    pub fn validate(&self, json: &str) -> ValidationResult {
        ...
        let value: Value = match serde_json::from_str(json) {
            ...
            Err(e) => {
                ...
                result
                    .errors
                    .push(ValidationError::SchemaValidation(format!(
                        "JSON parse error: {}",
                        e
                    )));
                return result;
    ...
    fn validate_structure(&self, value: &Value, result: &mut ValidationResult) {
        ...
        if !value.get("schema_version").is_some() {
            result
                .errors
                .push(ValidationError::MissingField("schema_version".to_string()));
        ...
        if value.get("training").is_none() {
            result
                .warnings
                .push("Missing 'training' field - training analysis will be skipped".to_string());
        }
        if value.get("hardware").is_none() {
            result
                .warnings
                .push("Missing 'hardware' field - hardware analysis will be skipped".to_string());
        }
    }
    ...
    fn validate_model(&self, model: &Value, result: &mut ValidationResult) {
        ...
        if let Some(name) = model.get("name") {
            ...
            result
                .errors
                .push(ValidationError::MissingField("model.name".to_string()));
        }
        ...
        if let Some(t) = model.get("type") {
            ...
            result
                .errors
                .push(ValidationError::MissingField("model.type".to_string()));
        }
        ...
        if let Some(params) = model.get("global_params") {
            ...
            for field in &["hidden_size", "num_layers"] {
                if let Some(v) = params.get(field) {
                    ...
                    result.errors.push(ValidationError::MissingField(format!(
                        "model.global_params.{}",
                        field
                    )));
                }
            ...
            if !["cnn", "diffusion"].contains(&model_type) && params.get("vocab_size").is_none() {
                result.warnings.push(
                    "global_params.vocab_size missing - may affect parameter calculation"
                        .to_string(),
                );
            }
            if !["cnn"].contains(&model_type) && params.get("sequence_length").is_none() {
                result.warnings.push(
                    "global_params.sequence_length missing - may affect FLOPs calculation"
                        .to_string(),
                );
            }
        } else {
            result.errors.push(ValidationError::MissingField(
                "model.global_params".to_string(),
            ));
        }
        ...
        if let Some(layers) = model.get("layers") {
            ...
            result
                .errors
                .push(ValidationError::MissingField("model.layers".to_string()));
        }
    }
        ...
        for (i, layer) in layers_arr.iter().enumerate() {
            ...
            if let Some(id) = layer.get("id").and_then(|v| v.as_str()) {
                if seen_ids.contains(id) {
                    result
                        .errors
                        .push(ValidationError::DuplicateLayerId(id.to_string()));
                }
    ...
    fn validate_hardware(&self, hardware: &Value, result: &mut ValidationResult) {
        ...
        if let Some(gpus) = hardware.get("gpus") {
            ...
            result
                .errors
                .push(ValidationError::MissingField("hardware.gpus".to_string()));
        }
        ...
        if result.metrics.gpu_count > 1 && hardware.get("interconnect").is_none() {
            result
                .warnings
                .push("Multiple GPUs configured but no interconnect specified".to_string());
        }
        if result.metrics.gpu_count > 1 && hardware.get("interconnect_bandwidth_gbs").is_none() {
            result.warnings.push(
                "Multiple GPUs configured but no interconnect_bandwidth_gbs specified".to_string(),
            );
        }
    }
    ...
    fn validate_training(&self, training: &Value, _root: &Value, result: &mut ValidationResult) {
        ...
        for field in &["batch_size", "optimizer", "precision"] {
            if training.get(field).is_none() {
                result
                    .errors
                    .push(ValidationError::MissingField(format!("training.{}", field)));
            }
        }
        ...
        if training.get("learning_rate").is_none() {
            result
                .warnings
                .push("training.learning_rate missing - using default".to_string());
        }
    ...
    fn check_aberrant_values(&self, value: &Value, result: &mut ValidationResult) {
                ...
                for gpu in gpus {
                    if let (Some(mem), Some(count)) = (
                        ...
                        if total_mem > 10000.0 {
                            result.warnings.push(format!(
                                "Total GPU memory ({:.0} GB) seems unusually high",
                                total_mem
                            ));
                        }
                    }
        ...
        if let Some(latency) = value
            ...
            .and_then(|v| v.as_f64())
        {
            if latency > 3600000.0 {
                // 1 hour in ms
                result.warnings.push(format!(
                    "Latency ({:.2} hours) seems unusually high",
                    latency / 3600000.0
                ));
            }
        }
        ...
        if let (Some(peak_mem), Some(gpu_mem)) = (
            value
                .get("memory")
                .and_then(|m| m.get("peak_vram_bytes"))
                .and_then(|v| v.as_u64()),
            value
                .get("hardware")
                .and_then(|h| h.get("gpus"))
        ...
        if let (Some(batch), Some(seq)) = (
            ...
            value
                .get("model")
                .and_then(|m| m.get("global_params"))
                .and_then(|p| p.get("sequence_length"))
                .and_then(|v| v.as_u64()),
        ) {
...
mod tests {
    ...
    fn test_shape_mismatch() {
        ...
        assert!(result
            .errors
            .iter()
            .any(|e| matches!(e, ValidationError::ShapeMismatch { .. })));
    }
}
