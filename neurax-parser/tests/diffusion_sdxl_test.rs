...
fn test_diffusion_sdxl_compilation() {
    ...
    let config = parse_model_config(DIFFUSION_SDXL_JSON).expect("Failed to parse Diffusion JSON");
    ...
    println!(
        "│ image_size:             {:>15}               │",
        grc.image_size.unwrap_or(0)
    );
    println!(
        "│ in_channels:            {:>15}               │",
        grc.in_channels.unwrap_or(0)
    );
    println!(
        "│ out_channels:           {:>15}               │",
        grc.out_channels.unwrap_or(0)
    );
    println!(
        "│ latent_channels:        {:>15}               │",
        grc.latent_channels.unwrap_or(0)
    );
    println!(
        "│ diffusion_timesteps:     {:>15}               │",
        grc.diffusion_timesteps.unwrap_or(0)
    );
    ...
    println!(
        "│ cross_attention_dim:    {:>15}               │",
        grc.cross_attention_dim.unwrap_or(0)
    );
    println!(
        "│ attention_head_dim:      {:>15}               │",
        grc.attention_head_dim.unwrap_or(0)
    );

    if let Some(ref block_ch) = grc.block_out_channels {
        println!(
            "│ block_out_channels:      {:>15?}              │",
            block_ch
        );
    }
    ...
    println!(
        "│ vae_scale_factor:        {:>15}               │",
        grc.vae_scale_factor.unwrap_or(8)
    );
    println!(
        "│ sample_size:             {:>15}               │",
        grc.sample_size.unwrap_or(0)
    );
    ...
    println!(
        "│ DALL-E 2       │ {:>8.2} │ {:>9.2} │ {:>10} │ {:>5} │",
        dalle2.unet_params_billion,
        dalle2.total_params_billion,
        dalle2.image_size,
        dalle2.diffusion_steps
    );
    println!(
        "│ SDXL (compiled)│ {:>8.2} │ {:>9.2} │ {:>10} │ {:>5} │",
        total_params as f64 / 1e9,
        total_params as f64 / 1e9,
        1024,
        1000
    ...
    println!(
        "│ Training samples:        {:>15.0}           │",
        training_samples
    );
    println!(
        "│ Diffusion steps:         {:>15.0}           │",
        diffusion_steps
    );
    println!(
...
fn test_diffusion_vs_real_models() {
    ...
    println!(
        "│ Latent Ch       │ {:>7} │ {:>6} │ {:>6} │ {:>12}        │",
        sd15.latent_channels,
        sdxl.latent_channels,
        sd3.latent_channels,
        grc.latent_channels.unwrap_or(0)
    );
    println!(
        "│ Diffusion Steps │ {:>7} │ {:>6} │ {:>6} │ {:>12}        │",
        sd15.diffusion_steps,
        sdxl.diffusion_steps,
        sd3.diffusion_steps,
        grc.diffusion_timesteps.unwrap_or(0)
    );
    println!("└────────────────────────────────────────────────────────────────────┘\n");
