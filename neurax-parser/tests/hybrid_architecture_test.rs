...
fn test_all_hybrid_models() {
    ...
    let models = [
        HybridModel {
            name: "ViT-L",
            json: VIT_HYBRID_JSON,
            families: &["CNN", "Transformer"],
        },
        HybridModel {
            name: "DiT-XL",
            json: DIT_HYBRID_JSON,
            families: &["Transformer", "Diffusion"],
        },
        HybridModel {
            name: "LSTM-Attn",
            json: LSTM_ATTENTION_HYBRID_JSON,
            families: &["RNN", "Transformer"],
        },
        HybridModel {
            name: "Mixtral-8x7B",
            json: MOE_TRANSFORMER_HYBRID_JSON,
            families: &["MoE", "Transformer"],
        },
        HybridModel {
            name: "ConvNeXt-Attn",
            json: CONVNEXT_ATTENTION_HYBRID_JSON,
            families: &["CNN", "Transformer"],
        },
        HybridModel {
            name: "Whisper-Large",
            json: WHISPER_HYBRID_JSON,
            families: &["CNN", "Transformer", "Diffusion"],
        },
    ];
...
fn test_moe_transformer_detailed() {
    ...
    let total_params = IrInjector::calculate_total_params(&absorbed);
    println!(
        "Total Parameters: {:.2}B (47B total, 13B active per token)\n",
