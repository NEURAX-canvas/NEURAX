...
fn test_massive_cnn_compilation() {
    ...
    let config = parse_model_config(MASSIVE_CNN_JSON).expect("Failed to parse massive CNN JSON");
    ...
    assert!(
        grc.symbol_table.contains_key("num_classes"),
        "num_classes in symbol table"
    );
