...
fn test_single_attention_block() {
    ...
    // ── Parse ──────────────────────────────────────────────────────────
    let config = parse_model_config(SINGLE_ATTENTION_JSON).expect("Failed to parse attention JSON");
    ...
    println!(
        "│ num_key_value_heads:    {:>10} (GQA enabled)        │",
    ...
    let kv_heads = grc
        .num_key_value_heads
        .unwrap_or(grc.num_attention_heads.unwrap()) as f64;
    let head_dim = grc.head_dim as f64;

    // Q projection: H x H
    let q_params = h * h;
    println!(
        "│ Q projection:           {:>12.0} params           │",
        q_params
    );
    ...
    println!(
        "│   num_kv_heads:         {:>10}                     │",
        mem_config.num_kv_heads
    );
    println!("├─────────────────────────────────────────────────────────────┤");
    println!("│ HardwarePass:                                           │");
    println!(
        "│   gpu_tflops_fp16:      {:>10.1}                     │",
        hw_config.gpu_tflops_fp16
    );
    println!(
        "│   has_tensor_cores:     {:>10}                     │",
        hw_config.has_tensor_cores
    );
    println!("└─────────────────────────────────────────────────────────────┘\n");
