...
fn test_transformer_1t_compilation() {
    ...
    let config =
        parse_model_config(TRANSFORMER_1T_JSON).expect("Failed to parse Transformer-1T JSON");
    ...
    println!(
        "│ num_key_value_heads:    {:>15} (GQA 8:1)     │",
    ...
    println!(
        "│ Memory BW (GB/s):       {:>15.1}               │",
        grc.primary_gpu_bw_gb_s
    );
    println!(
        "│ Tensor Cores:           {:>15}               │",
        grc.has_tensor_cores
    );
    println!(
        "│ Interconnect BW (GB/s): {:>15}               │",
        grc.interconnect_bw
    );
    ...
    println!("├─────────────────────────────────────────────────────────────┤");
    println!(
        "│ Expected params:         {:>15.2}B           │",
        expected_params
    );
    println!(
        "│ Calculated params:      {:>15.2}B           │",
        total_params as f64 / 1e9
    );
    ...
    let training_flops =
        RealModelSpecs::calculate_training_flops(total_params as f64 / 1e9, training_tokens);
    ...
    println!(
        "│ GPT-4 estimated:       {:>15.0}M GPU-hours   │",
        gpt4.gpu_hours_million
    );
    println!(
        "│ LLaMA-3-405B:          {:>15.0}M GPU-hours   │",
        llama3.gpu_hours_million
    );
    println!("└─────────────────────────────────────────────────────────────┘\n");
    ...
    println!(
        "│ Model weights:          {:>15.2} TB         │",
        params_bytes / 1e12
    );
    println!(
        "│ Optimizer states:       {:>15.2} TB         │",
        optimizer_bytes / 1e12
    );
    println!(
        "│ Gradients:              {:>15.2} TB         │",
        gradients_bytes / 1e12
    );
    println!(
        "│ Total (no sharding):    {:>15.2} TB         │",
    ...
    // ── Assertions ─────────────────────────────────────────────────────
    assert!(
        total_params > 500_000_000_000,
        "Expected > 500B params, got {}",
        total_params
    );
    assert!(
        total_params < 2_000_000_000_000,
        "Expected < 2T params, got {}",
        total_params
    );
...
fn test_transformer_1t_vs_real_models() {
    ...
    assert!(
        our_params > llama3.params_billion,
        "Should be larger than LLaMA-3-405B"
    );
    println!(
        "✓ Transformer-1T is larger than LLaMA-3-405B ({:.0}B params)",
        llama3.params_billion
    );
}
...
fn test_transformer_1t_flops_accuracy() {
    ...
    assert!(
        calculated_flops > 1e12,
        "FLOPs should be > 1 TFLOP per token"
    );
}
