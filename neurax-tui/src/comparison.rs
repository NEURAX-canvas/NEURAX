...
pub fn render_comparison_summary(
    result: &AnalysisResult,
    real: &RealWorldData,
    _area: Rect,
) -> Table<'static> {
    // Calculate overall accuracy
    let param_accuracy = calculate_accuracy(
        result.arch.metrics.total_parameters as f64,
        real.total_params as f64,
    );
    let memory_accuracy =
        calculate_accuracy(result.memory.metrics.peak_vram_gb(), real.peak_memory_gb);
    ...
    let overall_accuracy =
        (param_accuracy + memory_accuracy + latency_accuracy + throughput_accuracy + cost_accuracy)
            / 5.0;
    ...
    let rows = vec![
        Row::new(vec![
            ...
            Cell::from(format!("{:.1}%", param_accuracy))
                .style(Style::default().fg(accuracy_color(param_accuracy))),
        ])
        .style(Style::default()),
        Row::new(vec![
            ...
            Cell::from(format!("{:.1}%", memory_accuracy))
                .style(Style::default().fg(accuracy_color(memory_accuracy))),
        ]),
        Row::new(vec![
    ...
    .block(
        Block::default()
            .title(format!(
                " Overall Accuracy: {:.1}% | Source: {} ",
                overall_accuracy, real.source
            ))
            .borders(Borders::ALL)
            .title_style(
                Style::default()
...
pub fn render_metrics_count_summary(_result: &AnalysisResult, _area: Rect) -> Paragraph<'static> {
    ...
    Paragraph::new(vec![
        ...
        Line::from(vec![Span::raw(
            "  Architecture: 5  |  Graph: 6  |  Tensor: 9  |  Operator: 5",
        )]),
        Line::from(vec![Span::raw(
            "  Compute: 12  |  Memory: 11  |  Parallelism: 10",
        )]),
        Line::from(vec![Span::raw("  Hardware: 10  |  Cost: 9")]),
    ])
    .block(
        Block::default()
            .title("Metrics Count")
